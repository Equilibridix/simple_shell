...
int main(int argc, char **argv, char **envp)
{
	...
	while (1)
	{
		...
		while (alias_expansion(&b, env_p))
			;
		variable_expansion(&b, env_p, retrn_val);
		_getline_fileread(&b, env_p);
		tokenize_buf(&b, &arg_list);
		if (arg_list[0] == NULL)
			continue;
		retrn_val = run_builtin(arg_list, env_p, b.size);
		if (retrn_val != 0 && retrn_val != 2)
			retrn_val = run_execute(arg_list, env_p, b.size);
	}
	return (0);
}
 ...
 * @b: buffer structure
 * @return_val: Return value from last command
 ...
 */
int more_cmds(buffer *b, int return_val)
{
	if (b->bp == 0)
		return (0);
