...
/**
 * buff_word_erase - overwrite word from n to first whitespace
 * @b: pointer to buffer structure
...
/**
 * buff_insert - insert string into buffer at position n
 * @b: pointer to buffer structure
 * @s: string to insert
 ...
 */
void buff_insert(buffer *b, char *s, int n)
{
	unsigned int i, tot_len, s_len;
	int j;
	...
	{
		tot_len = b->bp;
		tot_len += _strlen(b->buf + b->bp);
		tot_len += s_len;
		while (tot_len > b->size)
			buff_reallocate(b);
